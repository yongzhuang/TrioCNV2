...
public class TrioViterbi {
    ...
    public TrioViterbi(List<Observation> observations, double[] PI, double[][][][][][] transition, double[][][] inheritance, Emission emission) {
...
//                    double oEmission = emissions[2].getProbOfObsGivenState(o, observations.get(0).getGC(), observations.get(0).getMappability(), observations.get(0).getRD(2));
                    int[] states=new int[] {f,m,o};
                    double emissionProb=emission.getProbOfRDEmission(states, observations.get(0));
                    r[0][f][m][o] = Math.log(PI[m]) + Math.log(PI[f]) + Math.log(PI[o]) + inheritance[f][m][o] + emissionProb;
                    path[0][f][m][o][0] = 0;
                    path[0][f][m][o][1] = 0;
                    path[0][f][m][o][2] = 0;
                }
...
//                        aEmission = fEmission + mEmission + oEmission;
                        ...
                        int[] states=new int[] {f,m,o};
                        double emissionProb=emission.getProbOfRDEmission(states, observations.get(t));                        
                        ...
                        for (int f1 = 0; f1 < NUM_STATES; f1++) {
                            for (int m1 = 0; m1 < NUM_STATES; m1++) {
                                for (int o1 = 0; o1 < NUM_STATES; o1++) {
                                    ...
                                    double tem = r[t - 1][f1][m1][o1] + emissionProb+ transition[f1][m1][o1][f][m][o];
                                    if (tem > tmp) {
                                        tmp = tem;
                                        s[0] = f1;
                                        s[1] = m1;
